<div class="modal-content row">
	<div class="col s12">
		<h5>Attach images to <b>@if($translated) {{$translated->name}} @else {{trans('main.misc.new_settings')}} @endif</b></h5>
	</div>
...
<script type="text/javascript">
	$(document).ready(function(e) {
        ...
        $(document).off('click', '#load_images_from_gallery-btn').on('click', '#load_images_from_gallery-btn', function (e) {
            manage_load("{{ route('settings-load_gallery_items') }}", $('#gallery_items-container'), {'menu_item_name': 'settings', 'layout_grid_gallery': 'l2 m3 s6', 'id': id, 'language_id': $('#current_lang_id').val()}, 300, null, 'post', function(e) {
        ...
        $(document).off('click', '#selectable-gallery .gallery-item-figure-inline').on('click', '#selectable-gallery .gallery-item-figure-inline', function (e) {
            ...
            var selected_ind = $(this).find('.figure-selected-indicator');

            $(this).toggleClass('selected');

            if ($(this).hasClass('selected')) {
                selected_ind.addClass('pulse-s');
            } else {
                selected_ind.removeClass('pulse-s');
            }
