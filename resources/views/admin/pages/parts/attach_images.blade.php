<div class="modal-content row">
	<div class="col s12">
		<h5>Attach images to <b>@if($translated) {{$translated->name}} @else {{trans('main.misc.new_pages')}} @endif</b></h5>
	</div>
...
<script type="text/javascript">
	$(document).ready(function(e) {
        ...
        $(document).off('click', '#load_images_from_gallery-btn').on('click', '#load_images_from_gallery-btn', function (e) {
            manage_load("{{ route('pages-load_gallery_items') }}", $('#gallery_items-container'), {'menu_item_name': 'pages', 'layout_grid_gallery': 'l2 m3 s6', 'id': id, 'language_id': $('#current_lang_id').val()}, 300, null, 'post', function(e) {
        ...
        $(document).off('click', '.item-attach_images-save-btn').on('click', '.item-attach_images-save-btn', function (e) {
            ...
            update_glob("{{route('pages-attach_images')}}", data, function() {
                manage_load("{{route('pages-load_attached_images')}}", container, data, true);   
            });
