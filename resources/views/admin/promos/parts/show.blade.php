                            ...
                            <div class="row search-tabs-row search-tabs-container custom-accent-color-1">
                                <div class="col l6 m6 s12">
                                    <div class=" bred-c-holder-1" >
                                        ...
                                        <a href="{{route('promos.index')}}">{{trans('main.breadcrumbs.promos')}}</a>
                                        <span><i class="material-icons">keyboard_arrow_right</i></span>
                                        <a href="{{route('promos.track', $promo->id)}}" style="font-size:25px;">{{ $promo->code }}</a>
                                    </div>
                                </div>
                    ...
                    <div class="logs-parameters-container">
                        <div class="row" style="padding:1.5rem 10px; border-radius: 2px; margin: 0;">
                            <div class="col l3 m5 s12">
                                <div><b>Статус: </b>@if($promo->status == 1 && $promo->date_end >= date('Y-m-d'))<span class="green-text" style="text-transform: uppercase;"><b>активен</b></span>@else <span class="red-text" style="text-transform: uppercase;"><b>не активен</b></span> @endif</div>
                                <div><b>Тип: </b>
                                    @if($promo->type == 1)
                                    ...
                                    @elseif($promo->type == 2 || $promo->type == 3)
                                        ...
                                        <span>{{ $disc_t_1 }} ({{ $promo->type_promo == 1 ? 'На один товар' : 'На все товары' }})</span>
                                    @endif
                                </div>
                                <div><b>Дата генерации: </b>{{ $promo->date_start }}</div>
                                <div><b>Действителен до: </b>{{ $promo->date_end }}</div>
                                <div><b>Количество использований: </b> @if($promo->limit == 0) {{ $promo->log()->count() }} @else {{ $promo->log()->count() . ' из ' . $promo->limit}} @endif</div>
                            </div>
                            <div class="col l9 m7 s12">
                                <div class="table-info-wrapper-reverse">
                                    <b>Скидка: </b>
                                    @if($promo->type == 1)
                                        <div class="chip">{{ $promo->percent.'%' }}</div>
                                    @elseif($promo->type == 2 || $promo->type == 3)
                                        @php($products_percent = json_decode($promo->products_percent, true))
                        ...
                        <div class="load-filter-container row deep-purple lighten-5">
                            ...
                            <div class="input-field col l2 m3 s12" style="margin-top: 0;">
                                <div class="input-field col s12">
                                    ...
                                    <label for="log_user" class="input-label-1">{{ trans('main.promos.log.filter.search_user') }}</label>
                                </div>
                            </div>
                            <div class="input-field col l2 m3 s12" style="margin-top: 0;">
                                <div class="input-field col s12">
                                    <input id="log_order" type="text" class="validate" name="log_order">
                                    <label for="log_order" class="input-label-1">{{ trans('main.promos.log.filter.search_order') }}</label>
                                </div>
                            </div>
...
<script type="text/javascript">
	...
	$(document).ready(function() {
        ...
        var logs_datatable = $('#logs-datatable').DataTable({
            ...
            "responsive": true,
            columns: [
                {"name":"date_used","data":"date_used","orderable":true,"searchable":true},
