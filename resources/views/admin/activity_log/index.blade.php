...
@section('title', 'Activity Log - Home')
                            ...
                            <div class="row search-tabs-row search-tabs-container custom-accent-color-1">
                                <div class="col l6 m6 s12">
                                    <div class=" bred-c-holder-1" >
                                        ...
                                        <a href="{{route('home-index')}}">{{trans('main.breadcrumbs.home')}}</a> <span><i class="material-icons">keyboard_arrow_right</i></span> <a href="{{route('activitylog.index')}}" style="font-size:25px;">{{trans('main.breadcrumbs.activitylog')}}</a>
                                    </div>
                                </div>
                        ...
                        <form method="POST" id="search-form" class="form-inline" role="form" action="">
                            <div class="load-filter-container row deep-purple lighten-5">
                                ...
                                <div class="input-field col l2 m3 s6">
                                    <select multiple name="region[]" id="activity_log-region-select">
                                    ...
                                    <label>{{trans('main.users.table.filter.region.header')}}</label>
                                </div>
                                <div class="input-field col l2 m3 s6" style="padding-left: 0;">
                                    <select name="type" id="activity_log-type-select">
                                      <option value="" selected>{{trans('main.misc.any')}}</option>
                                      <option value="allow">Granted permissions</option>
                                      <option value="deny">Denied permissions</option>
                                    </select>
                                    <label style="left:0;">Type</label>
                                </div>
	...
	(function(window,$){
		...
		window.LaravelDataTables["dataTableBuilder"] = $("#dataTableBuilder").DataTable({
			...
			"columns":[
                ...
                {"name":"created_at","data":"created_at","title":"{{trans('main.activity_log.table.header.date')}}","orderable":true,"searchable":true},
                {"name":"user", "data": 'user', "title":"{{trans('main.activity_log.table.header.user')}}","orderable":false,"searchable":false},
                {"name":"ip","data":"ip","title":"{{trans('main.activity_log.table.header.ip')}}","orderable":true,"searchable":true},
    			{"name":"region_id", "data": 'region', "title":"{{trans('main.activity_log.table.header.region')}}","orderable":true,"searchable":true},
    			{"name":"permission","data":"permission","title":"{{trans('main.activity_log.table.header.permission')}}","orderable":false,"searchable":false},
                {"name":"method","data":"method","title":"{{trans('main.activity_log.table.header.method')}}","orderable":true,"searchable":true},
    			{"name":"message","data":"message","title":"{{trans('main.activity_log.table.header.message')}}","orderable":true,"searchable":true},
                {"name":"type","data":"type","title":"{{trans('main.activity_log.table.header.type')}}","orderable":true,"searchable":true},	
			],
        ...
        $(document).on('change', '#activity_log-region-select',function(e) {
        ...
        $(document).on('change', '#activity_log-type-select',function(e) {
            type_value = $(this).val();
            delay(function(){
              window.LaravelDataTables["dataTableBuilder"].draw();
            }, 200 );
