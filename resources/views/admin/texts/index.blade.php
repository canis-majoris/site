                            ...
                            <div class="row search-tabs-row search-tabs-container custom-accent-color-1 ">
                                <div class="col l6 m6 s12 hide-on-med-and-down">
                                    <div class=" bred-c-holder-1" >
                                        <a href="{{route('home-index')}}">{{trans('main.breadcrumbs.home')}}</a><span><i class="material-icons">keyboard_arrow_right</i></span><a href="{{route('texts.index')}}" style="font-size:25px;">{{trans('main.breadcrumbs.documents')}}</a>
                                    </div>
                                </div>
            ...
            <div class="side-holder-next opened-1" id="">
                <div class="col s12 m4 l3 left-fixed-small-1 side-slidabl-wrapper opened-1" style="bottom: -15px;">
                    <div class="side-slidable-left-1-top">
                        <button class="btn btn-flat side-slidable-toggle-btn tooltipped" data-position="right" data-delay="50" data-tooltip="{{ trans('main.content.sidebar.texts_types.toggle_button_toolptip') }}"><i class="material-icons">keyboard_arrow_left</i></button>
                    </div>
                            ...
                            <div class="col s12">
                                <div class="">
                                    <div id="menu_items-container" style="margin: 0 -22px; padding: 20px 0;" class="side-menu-items-wrapper">
                                        @include('texts.parts.form_right')
                                    </div>
                ...
                <div class="col l9 m8 s12 right side-content-wrapper-1">
                    <div class="white shadow-1-1 animate-c-wrapper">
                        <div class="loading-block animate-c animate-left-out visible-c" id="menu_item_data-container">
                            @include('texts.parts.form_right_data')
                        </div>
...
<script type="text/javascript">
    ...
    var side_menu_load_url = "{{route('texts_types-load')}}",
        side_menu_load_settings_url = "{{ route('texts_types-load_settings') }}",
        side_menu_item_edit_url = "{{ route('texts_types-edit') }}",
        side_menu_item_update_url = "{{route('texts_types-update')}}",
        side_menu_item_delete_url = "{{route('texts_types-delete')}}",
        item_edit_url = "{{route('texts-edit')}}",
        item_create_url = "{{route('texts-new')}}",
        item_delete_url = "{{route('texts-delete')}}",
        item_change_status_url = "{{route('texts-changestatus')}}",
    ...
    (function(window,$){
        ...
        window.LaravelDataTables["dataTableBuilder"] = $("#dataTableBuilder").DataTable({
            ...
            "columns":[
                ...
                {"name":"name","data":"name","title":"{{ trans('main.content.texts.table.header.name') }}","orderable":true,"searchable":true},
                {"name":"name_trans","data":"name_trans","title":"{{ trans('main.content.texts.table.header.name_trans') }}","orderable":true,"searchable":false},
                {"name":"created_at","data":"created_at","title":"{{ trans('main.content.texts.table.header.date') }}","orderable":true,"searchable":true},
                {"name":"img","data":"img","title":"{{ trans('main.content.texts.table.header.image') }}","orderable":false,"searchable":false},
                {"name":"watch","data":"watch","title":"{{ trans('main.content.texts.table.header.watch') }}","orderable":false,"searchable":false, width:'50px'},
        ...
        $(document).on('click', '.texts-edit-btn', function(e) {
            e.preventDefault();
            item_edit(this, function () {});
        });

        $(document).on('click', '.texts-delete-btn', function(e) {
            e.preventDefault();
            delete_item(this, texts, function () {});
        });

        $(document).on('click', '.texts_type-delete-btn', function (e) {
            e.stopPropagation();
