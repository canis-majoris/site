                ...
                swal.queue([{
                  ...
                  preConfirm: () => {
                    ...
                    $(wrapper).fadeOut(100, function() {
                        update_glob("{{route('texts-remove_img')}}", {'id': "{{$item->id}}", 'language_id': $('#current_lang_id').val(), 'img': img}, function() {
                            manage_load("{{route('texts-load_attached_images')}}", container, data);
